...
// Setting table (Settings → Keys → Storage), env vars are the fallback, no SDK
...
function requireAccess(access: DriveAccess | null): DriveAccess {
  if (!access) {
    throw new Error(
      "Google Drive storage is selected but not connected — set it up under Settings → Keys → Storage" +
