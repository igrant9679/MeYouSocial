...
export const HELP_CATEGORIES: FaqCategory[] = [
  {
    ...
    entries: [
      ...
      },
      {
        ...
        a: "The rail is the loop, in the order work moves. Top to bottom: **Inbox** (everything waiting on a person — the landing page), **Assistant**, then the six stages — **Research** (Intel, bookmarks, competitors), **Ideas** (the one board, plus keywords and experts), **Drafts** (articles, and the video studio when it is on), **Publish** (approvals, content audit, website, blog calendar), **Distribute** (compose, calendar, engage), **Measure** (reports, insights, analytics) — then the **Setup** group for admins (**Settings**, **Channels**, **Brand**), and **Help**. At the bottom: your profile and sign out. Inside a stage, a strip along the top shows its Overview and tabs with count badges.\n\nYou don't have to memorise any of it — **hover any rail entry and a bubble tells you what that module is for**. The rail collapses to icons on a narrow window, and the same hover still names it.",
      ...
      },
      {
        ...
        links: [{ label: "Settings → Keys →", href: "/admin/api-keys" }],
      ...
      },
      {
        ...
        links: [{ label: "Settings → Keys →", href: "/admin/api-keys" }, { label: "Settings → Connections →", href: "/admin/connections" }],
      ...
      },
      {
        ...
        links: [{ label: "Settings → Keys →", href: "/admin/api-keys" }, { label: "Thumbnail Studio →", href: "/thumbnails" }, { label: "Blog → Brand →", href: "/blog/brand" }],
  ...
  },
  {
    ...
    entries: [
      {
        q: "Where did the Blog section go?",
        a: "There isn't one any more - the blog pages were split across the stages that own them, and each kept its URL.\n\n**Articles** live under **Drafts** (`/blog`), as one page with a **Board / List** toggle: the board is the four-column pipeline, the list is every article newest-edit-first with its unverified-citation count. Cards open the editor either way.\n\nThe rest moved to the stage that matches the work: **Ideas** owns the idea board, Keywords and Experts; **Publish** owns Approvals, the content Audit, Website and the blog calendar; **Measure** owns Analytics and the monthly Report; **Brand** owns the 7 Motifs and Organization; **Settings** owns Automation and every operator page that used to sit under its own menu.\n\nNothing was deleted and no link broke - `/blog/board` opens the list view, and the old Blog tab-strip URLs all still resolve.",
        links: [{ label: "Open Articles →", href: "/blog" }, { label: "The list view →", href: "/blog?view=list" }],
        tags: ["blog", "workspace", "tabs", "kanban", "articles", "board", "list"],
      },
      {
  ...
  },
  {
    ...
    entries: [
      ...
      },
      {
        ...
        a: "Settings → **Keys** → *Media & video*: paste a Google key and set the **Video renderer** to Auto or Veo. Auto uses Veo whenever a key is present; Mock never spends. Real renders count against the daily cap shown on the Videos page, and outputs are persisted to storage because Veo links expire in ~2 days.",
        links: [{ label: "Settings → Keys →", href: "/admin/api-keys" }],
      ...
      },
      {
        ...
        links: [{ label: "Distribute →", href: "/distribute" }, { label: "Settings → Keys →", href: "/admin/api-keys" }],
      ...
      },
      {
        ...
        links: [{ label: "Brand hub →", href: "/brand" }, { label: "Settings → Keys →", href: "/admin/api-keys" }],
        tags: ["branded short", "heygen", "hyperframes", "brand", "cost"],
      },
      {
        q: "Can it generate a voiceover?",
        a: "Yes — the **Generate voiceover** button on a storyboard reads the scene texts as a narration script. With the mock TTS it stores that script as clearly-labeled text; paste an ElevenLabs key and switch the TTS provider (Settings → Keys) for real audio.",
    ...
    entries: [
      {
        ...
        a: "Settings → **People** → enter email + role (Admin / Editor / Viewer) → **Send invitation**. They get an email link; on accept, they join your workspace.",
        tags: ["invite", "team"],
      },
      {
      ...
      {
        q: "Is there a cost?",
        a: "No. MeYouSocial Publisher has no billing, no credits, no payments. AI usage is unmetered for invited members. Admins can optionally set soft monthly limits per user (under Settings → Usage) to bound shared infrastructure cost.",
        tags: ["cost", "billing", "limits"],
      },
      {
        q: "How do soft limits work?",
        a: "Settings → Usage → set caps for scripts/month, thumbnails/month, agent runs/month, channels per workspace. Leave blank or 0 for unlimited. They're operational guards, never a paywall.",
        tags: ["limits", "caps"],
      },
      {
        q: "Can several companies share one install?",
        a: "Yes — each company lives in its own **workspace** with fully separate content, team, API keys, SMTP and branding. Signing up creates your company's workspace (rename it under Settings → Workspace); invite teammates from Admin. Someone invited to your workspace who signs up via the invite link joins **your** workspace directly. People who belong to several companies get a workspace switcher in the header.",
        tags: ["multi-tenant", "companies", "workspaces", "teams"],
      },
      {
        q: "Whose API keys does my workspace use?",
        a: "Your own, when you've pasted them: everything under Settings → **Keys** (LLMs, search, ElevenLabs, image/video/TTS switches) is saved **per workspace**. If your workspace hasn't set a key, it falls back to the platform's shared key — the card shows which one is in effect. The one deliberate exception is the **YouTube Data API key**: it only reads public data, so a single platform-provided key serves every workspace and its card says so (there's nothing for you to set up). Same for **SMTP**: your notification and invitation emails go out through the server you configure under Settings → Email, visible only to your workspace.",
        links: [{ label: "Settings → Keys →", href: "/admin/api-keys" }, { label: "Settings → Email →", href: "/admin/email" }],
      ...
      },
      {
        ...
        a: "Settings → **Workspace** → *Branding*: pick an accent color (presets or any hex) and upload a logo. The whole app chrome — buttons, active states, sidebar mark and menu — re-tints for members of your workspace only; other companies keep their own look. Note this styles the **app**; the brand used in generated blog content lives separately under Blog → Brand.",
        links: [{ label: "Settings → Workspace →", href: "/admin/settings" }],
  ...
  },
  {
    ...
    entries: [
      ...
      },
      {
        ...
        a: "Almost certainly not — it needs published content and connected analytics before it has anything to say. Connect Search Console, GA4 or YouTube under Settings → Analytics, and publish a few posts; the panels fill in as the data arrives.",
        links: [{ label: "Settings → Analytics →", href: "/admin/analytics" }],
    ...
    entries: [
      {
        ...
        links: [{ label: "Settings → Keys → Storage →", href: "/admin/api-keys#storage" }],
  ...
  },
  {
    ...
    entries: [
      ...
      {
        q: "What is the status pill in the header telling me?",
        a: "It answers one question - is the engine working, and what did it last do? - in a sentence: **\"Published an article - 12:02\"**, or **\"Drafted an article\"**, or **\"Nothing has run yet\"**. The coloured dot is the tone of that last event: green done, amber in progress, red something failed.\n\nIf the global pause is on it says **Paused** and nothing else, because that is the only fact that matters until you resume it.\n\n**Click it** for the rest: the last dozen things the engine did, with their full titles, real times and links straight to what it made - plus whether it is running on its own (full autonomy) or waiting for you at the checkpoints you set.\n\n\"Nothing has run yet\" is not a failure. An idle autopilot writes no audit rows on purpose, so a quiet workspace genuinely has nothing to show.\n\nIt replaced a scrolling ticker of all-caps tokens that nobody could read: same information, fewer hieroglyphs.",
        links: [{ label: "Change what runs by itself →", href: "/setup/automation" }],
        tags: ["status", "pill", "header", "ticker", "engine", "activity", "live"],
      },
      {
