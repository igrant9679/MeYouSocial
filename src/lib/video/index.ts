...
const veoProvider: VideoProvider = {
  ...
  async render(req) {
    ...
    if (!apiKey) throw new Error("No Google API key configured (Settings → Keys → Google)");
...
// auto | mock | veo, set under Settings → Keys). Env USE_MOCK_VIDEO stays as
...
export async function getVideoProvider(workspaceId?: string): Promise<VideoProvider> {
  ...
  if (setting === "veo") {
    if (!key) throw new Error("Video provider is set to Veo but no Google key is configured (Settings → Keys)");
    return veoProvider;
  }
