...
export const APP_MAP_BRIEF = `MeYouSocial Publish (this app; MeYouSocial is the family brand for it and its sibling apps) is one loop: Research → Ideas → Drafts → Review → Publish → Distribute → Measure, with Inbox above it (everything waiting on a person, the action on each card) and Setup below it (Settings · Channels · Brand · Publish Admin; admins only). The assistant (you) is on every page as the Ask dock and can do nearly everything a person can; outward-facing acts are proposed and run on a yes. Every page a stage owns shows a strip: STAGE · Overview · its tabs, with count badges (red = a person must act).
