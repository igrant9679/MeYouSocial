...
export const APP_MAP_BRIEF = `MeYouSocial Publisher (this app; MeYouSocial is the family brand for it and its sibling apps) is one loop: Research → Ideas → Drafts → Publish → Distribute → Measure, with Inbox above it (everything waiting on a person, the action on each card — reviewing IS the Inbox, there is no separate Review stage) and Setup below it (Settings · Channels · Brand; admins only — the operator pages are tabs of Settings). The assistant (you) is on every page as the Ask dock and can do nearly everything a person can; outward-facing acts are proposed and run on a yes. Every page a stage owns shows a strip: STAGE · Overview · its tabs, with count badges (red = a person must act).
Stages and tabs: Research (Intel, Bookmarks, Competitors) · Ideas — one board for article and video ideas, four columns Discovered/Approved/Drafted/Rejected (tabs Keywords, Experts) · Drafts (Articles — one page with a Board/List toggle; Scripts, Thumbnails, Videos, Production when the video studio is on) · Publish (Approvals, Audit, Website, Blog calendar; Download HTML and Mark as published when there is no WordPress) · Distribute (Compose, Calendar, Engage) · Measure (Reports, Insights, Blog analytics, Blog report, Social performance, YouTube audit — the own-channel audit from the YouTube Analytics API) · Settings by question: People (roles, invitations, require approval), Automation (full autonomy, pause, weekly article target, publish day, SEO on drafts, social auto-dials, function modes), Schedule (slots, timezone, best time, UTM, campaigns), Connections (what is connected and what is missing), and for admins Keys, Analytics, Email, Workspace, Usage · Brand (Tone & motifs, Organization).
...
export const APP_GUIDE: GuideTopic[] = [
  {
    ...
    body: `Rail, top to bottom: Inbox (landing page: cards for posts awaiting approval, questions only you can answer, claims with no source, images that need your eye, articles held at review, invitations — then the pipeline beneath) · Assistant (also the Ask dock top-right on every page, Ctrl+/) · Research · Ideas · Drafts · Publish · Distribute · Measure · Setup group (Settings, Channels, Brand — admins only) · Help.
Each stage page has an Overview (its own rows with the verb on the row) and tabs to the module pages, which kept their URLs: Research → /intel, /intel/bookmarks, /channels/<id>/competitors (the old /chat is the assistant). Ideas → /ideas is the one board; tabs /blog/keywords, /blog/experts. Drafts → /blog (ONE Articles page, ?view=board default or ?view=list), and when the video studio is on /channels/<id>/scripts, /thumbnails, /videos, /production. Publish → /social/approvals, /blog/audit, /website, /blog/calendar. Distribute → /social/compose, /social/calendar, /social/engage. Measure → /reports, /insights, /blog/analytics, /blog/report, /social/performance, /youtube (channel audit). Settings → /setup with People, Automation, Schedule, Connections, and for admins Keys (/admin/api-keys), Analytics, Email, Workspace and Usage — the operator pages kept their /admin/* URLs and became tabs here. Brand → /brand, /blog/brand (tone & motifs), /blog/organization.
Old links still work: /dashboard → Inbox; /review → Inbox; /blog/board → /blog?view=list; /admin → Settings → People; /blog/ideas and a channel's ideas → the board; /blog/automation → Settings → Automation; /social/settings → Settings → Schedule; /social → Distribute. Personal settings (theme, text size, profile) are under /settings, not the workspace's Settings.`,
  },
  {
    id: "daily",
