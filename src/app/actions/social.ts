...
export async function createSocialPostAction(formData: FormData) {
  ...
  if (accountIds.length > 0 && accounts.length === 0) backTo("Those accounts aren't connected. Connect one under Settings → Connections.");
...
export async function updateSocialPostAction(formData: FormData) {
  ...
  if (accounts.length === 0) backTo("Those accounts aren't connected. Connect one under Settings → Connections.");
