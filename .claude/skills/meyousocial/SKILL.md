...
**One loop, seven stages.** Research → Ideas → Drafts → Review → Publish → Distribute → Measure. The left rail is that loop, in order, with **Inbox** above it (what needs a person, one card per item, the action on the card) and **Setup** below it (Settings · Channels · Brand · Admin, admins only). **Assistant** and **Help** sit outside the loop.
...
| Assistant `/assistant` | A chat that does the work — reads and drafts; cannot publish, send, schedule, queue, approve, delete or configure. | Threads. Every stage also has an **Ask** box at the bottom that opens a new thread. |
| Research `/research` | Competitor intelligence. | Strong outliers (≥2× their channel's average) with "Make it an idea". Tabs: Intel · Bookmarks · Competitors (active channel) · Chat. |
...
- Research: `/intel` (index channels by @handle or keyword; outliers), `/intel/channels/[id]`, `/intel/videos/[id]` (embed, transcript, chat-with-video), `/intel/bookmarks`, `/chat`, `/chat/[id]`, `/channels/[id]/competitors`, `/channels/[id]/research`.
...
5. Optional: ask the Assistant "what needs my attention?" — it reads the same data and can do the next step (draft, fill SEO) without ever publishing.
